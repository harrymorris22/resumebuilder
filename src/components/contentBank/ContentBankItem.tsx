...
export function ContentBankItemCard({ item, onDelete }: ContentBankItemCardProps) {
  return (
    <div
      className={`p-3 rounded-lg border border-stone-200 dark:border-stone-700 text-sm ${
        item.superseded ? 'opacity-50' : ''
      }`}
    >
      <div className="flex items-start justify-between gap-2">
        <p className="text-stone-800 dark:text-stone-200 flex-1">{item.text}</p>
        <button
          onClick={onDelete}
          className="text-stone-300 hover:text-rose-500 transition-colors flex-shrink-0 mt-0.5"
      ...
      <div className="flex items-center gap-2 mt-2 flex-wrap">
        ...
        {item.tags.map((tag) => (
          <span
            key={tag}
            className="px-1.5 py-0.5 text-xs rounded bg-stone-100 dark:bg-stone-700 text-stone-500 dark:text-stone-400"
          >
            {tag}
          </span>
        ))}
